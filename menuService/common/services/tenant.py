...
# 🟢 FIX 1: Standardized regex to match your middleware exactly
TENANT_REGEX = re.compile(r"^rest_[a-z0-9]+$")

# 🟢 FIX 2: Dynamically pull the service name so this works in any container
...
def provision_tenant(tenant_id: str):
    ...
    # Construct the target schema (e.g., 'menu_rest_123' or 'cart_rest_123')
    ...
    try:
        ...
        # If the pooler reuses this connection for another user without this reset, 
        # that user will accidentally read/write to the newly created schema!
        with connection.cursor() as cursor:
