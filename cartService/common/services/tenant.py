...
# Use the exact same regex as your middleware to ensure consistency
TENANT_REGEX = re.compile(r"^rest_[a-z0-9]+$")

# 🟢 FIX 1: Dynamically pull the service name so the common code works everywhere
...
def provision_tenant(tenant_id: str):
    ...
    # 🟢 FIX 2: Construct the target schema (e.g., 'cart_rest_123')
    ...
    try:
        ...
        # Django will build the tables inside the target_schema.
        call_command("migrate", interactive=False)
        
    finally:
        # 🟢 FIX 3: THE POOLER SAFETY NET
        # You MUST reset the search path back to public after migrating.
        # Otherwise, the transaction pooler will hand this connection to another
        # user, and they will accidentally read/write to this new tenant's schema!
        with connection.cursor() as cursor:
...
def deprovision_tenant(tenant_id: str):
    ...
    # Construct the target schema (e.g., 'cart_rest_123')
    target_schema = f"{SERVICE_NAME}_{tenant_id}"
